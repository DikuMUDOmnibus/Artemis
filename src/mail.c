...
mail_index_type *find_char_in_index(long searchee)
{
  ...
  if (searchee < 0) {
    logs("SYSERR: Mail system -- non fatal error #1.");
    return 0;
  }
...
void write_to_file(void *buf, int size, long filepos)
{
  ...
  if (filepos % BLOCK_SIZE) {
    logs("SYSERR: Mail system -- fatal error #2!!!");
    no_mail = 1;
    return;
  }
...
void read_from_file(void *buf, int size, long filepos)
{
  ...
  if (filepos % BLOCK_SIZE) {
    logs("SYSERR: Mail system -- fatal error #3!!!");
    no_mail = 1;
    return;
  }
  fseek(mail_file, filepos, SEEK_SET);
...
void index_mail(long id_to_index, long pos)
{
  ...
  if (id_to_index < 0) {
    logs("SYSERR: Mail system -- non-fatal error #4.");
    return;
  }
...
int scan_file(void)
{
  ...
  if (!(mail_file = fopen(MAIL_FILE, "r"))) {
    logs("Mail file non-existant... creating new file.");
  ...
  if (file_end_pos % BLOCK_SIZE) {
    logs("SYSERR: Error booting mail system -- Mail file corrupt!");
    logs("SYSERR: Mail disabled!");
    return 0;
  }
...
void store_mail(long to, long from, char *message_pointer)
{
  ...
  if (from < 0 || to < 0 || !*message_pointer) {
    logs("SYSERR: Mail system -- non-fatal error #5.");
    return;
  }
   ...
   header (i.e. the text handed to the player) */
{
  ...
  if (recipient < 0) {
    logs("SYSERR: Mail system -- non-fatal error #6.");
    return 0;
  }
  if (!(mail_pointer = find_char_in_index(recipient))) {
    logs("SYSERR: Mail system -- post office spec_proc error?  Error #7.");
    return 0;
  }
  if (!(position_pointer = mail_pointer->list_start)) {
    logs("SYSERR: Mail system -- non-fatal error #8.");
    return 0;
  }
  ...
  if (header.block_type != HEADER_BLOCK) {
    logs("SYSERR: Oh dear.");
    no_mail = 1;
    logs("SYSERR: Mail system disabled!  -- Error #9.");
    return 0;
  }
