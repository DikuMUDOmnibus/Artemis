...
int pop(struct stack_data * stack)
{
  if (S_LEN(stack) > 0)
    ...
    logs("Illegal expression in shop keyword list");
    return (0);
  }
}
...
int evaluate_expression(struct obj_data * obj, char *expr)
{
  ...
  while (*ptr) {
	...
	if (temp == OPER_CLOSE_PAREN) {
	  if ((temp = pop(&ops)) != OPER_OPEN_PAREN) {
	    logs("Illegal parenthesis in shop keyword expression");
	    return (FALSE);
	  }
	} else
	  push(&ops, temp);
  ...
  temp = pop(&vals);
  if (top(&vals) != NOTHING) {
    logs("Extra operands left on shop keyword expression stack");
    return (FALSE);
  }
  return (temp);
}
