...
char *spells[] =
{
  ...
  "moremana 100",	/* 70 */
  "moremana 75",
  "moremana 50", "!UNUSED!", "!UNUSED!", "!UNUSED!",	/* 75 */
  ...
  "steal",
  "track",			/* 140 */
  "shoot", "trip", "headbutt", "bodyslam", "doorbash",	/* 145 */
  "groinkick", "!UNUSED!", "!UNUSED!", "!UNUSED!", "!UNUSED!",	/* 150 */
	     ...
	     struct obj_data * ovict, int spellnum, int level, int casttype)
{
  ...
  if (IS_SET(SINFO.routines, MAG_MANUAL))
    ...
    case SPELL_LIGHTNING_BREATH:
				MANUAL_SPELL(spell_lightning_breath); break;*/
		          ...
		          char *argument)
{
  ...
  case ITEM_POTION:
    ...
    break;
  default:
    logs("SYSERR: Unknown object_type in mag_objectmagic");
    break;
  }
}
...
int is_being_attacked(struct char_data *ch) {
  int result = 0;
  ...
  if (result)
    if (number(0, 100) < MAX(25, MIN(75, GET_LEVEL(ch))))
      result = 0;
...
ACMD(do_cast)
{
  ...
  if  (GET_LEVEL(ch) >= LVL_IMMORT) {
	if (tch)
        	sprintf(buf, "(GC) %s has cast %s on %s.",GET_NAME(ch),
		spells[spellnum], GET_NAME(tch));
        if (tobj)
		sprintf(buf, "(GC) %s has cast %s on %s.",
			GET_NAME(ch), spells[spellnum], tobj->short_description);
        logs(buf);
/* I like this better :) */
  }

  /* You throws the dice and you takes your chances.. 101% is total failure */
  if ((number(0, 101) > GET_SKILL(ch, spellnum)) || is_being_attacked(ch)) {
    WAIT_STATE(ch, PULSE_VIOLENCE);
...
void mag_assign_spells(void)
{
  ...
  spello(SPELL_STARBURST, 80, X, X,  X,  80,  50,  1,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);

  spello(SPELL_POWER_WORD_STUN, X, 103, X, X, 80, 50, 1,
         POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);

  spello(SPELL_SUPER_HEAL, X, 55, X, X, 80, 60, 2,
	 POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS | MAG_UNAFFECTS); 

  spello(SPELL_WATERWALK, 15, 25, X,  X,  40,  20,  2,
         POS_STANDING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);

  spello(SPELL_FARSEE, 45, 55, X,  X,  40,  20,  2,
         POS_STANDING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);
  ...
  spello(SPELL_VIGORISE_LIGHT, X, 1, X, X, 30, 5, 2,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_VIGORISE_SERIOUS, X, 4, X, X, 30, 10, 2,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_VIGORISE_CRITIC, X, 9, X, X, 30, 15, 2,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_VIGORISE_GODLY, X, 199, X, X, 930, 815, 92,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_VIGORISE_100, X, 139, X, X, 830, 715, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_VIGORISE_75, X, 129, X, X, 730, 615, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_VIGORISE_50, X, 119, X, X, 630, 515, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_HEAL_GODLY, X, 199, X, X, 1930, 1515, 92,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_HEAL_100, X, 179, X, X, 1430, 915, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_HEAL_75, X, 159, X, X, 1230, 915, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_HEAL_50, X, 139, X, X, 930, 815, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_MOREMANA_GODLY, X, 199, X, X, 1930, 1815, 92,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_MOREMANA_100, X, 179, X, X, 1630, 1515, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_MOREMANA_75, X, 159, X, X, 1530, 1415, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_MOREMANA_50, X, 139, X, X, 1430, 1315, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_MOREMANA_25, X, 102, X, X, 1230, 1015, 72,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_CURE_LIGHT, X, 1, X, X, 30, 5, 2,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_CURE_SERIOUS, X, 4, X, X, 30, 10, 2,
         POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);

  spello(SPELL_CURE_CRITIC, X, 9, X, X, 30, 15, 2,
	 POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);
  /* C L A S S E S      M A N A   */
  /* Ma  Cl  Th  Wa   Max Min Chn */
  ...
  spello(SPELL_DETECT_ALIGN, 210, 4, 210, 210, 20, 10, 2,
  ...
  /* C L A S S E S      M A N A   */
  /* Ma  Cl  Th  Wa   Max Min Chn */
  spello(SPELL_DETECT_POISON, 10, 3, 210, X, 15, 5, 1,
  ...
  spello(SPELL_EARTHQUAKE, X, 30, X, X, 90, 60, 1,
  ...
  spello(SPELL_STRENGTH, 6, X, X, 110, 95, 80, 1,
	 POS_STANDING, TAR_CHAR_ROOM, FALSE, MAG_AFFECTS);
  ...
  spello(SPELL_SENSE_LIFE, 60, 50, X, X, 20, 10, 2,
	 POS_STANDING, TAR_CHAR_ROOM | TAR_SELF_ONLY, FALSE, MAG_AFFECTS);

  spello(SPELL_IDENTIFY, 60, 70, 150, 150, 50, 30, 1, POS_STANDING,
  ...
  /* Ma  Cl  Th  Wa  */
  spello(SKILL_BACKSTAB, 290, X, 3, X,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_BASH, X, 210, X, 12,
	 0, 0, 0, 0, 0, 0, 0);
  ...
  spello(SKILL_PICK_LOCK, 240, X, 2, X,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_RESCUE, X, X, 122, 3,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_SNEAK, 230, X, 1, X,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_STEAL, 110, X, 4, X,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_TRIP, 220, X, 10, X,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_HEADBUTT, X, X, X, 35,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_BODYSLAM, X, X, X, 75,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_DOORBASH, X, X, X, 55,
	 0, 0, 0, 0, 0, 0, 0);

  spello(SKILL_GROIN_KICK, X, X, X, 90,
	 0, 0, 0, 0, 0, 0, 0);
  ...
  spello(SKILL_SHOOT, 70, 70, 70, 70,
	 0, 0, 0, 0, 0, 0, 0);
  spello(SPELL_FIRE_BREATH, 0, 0, 0, 0, 0, 0, 0,
	 POS_STUNNED, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_MANUAL);
  ...
  spello(SPELL_LIGHTNING_BREATH, 0, 0, 0, 0, 0, 0, 0,
	 POS_STUNNED, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_MANUAL);
  spello(SPELL_FIREBALL_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_COLOR_SPRAY_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_MAGIC_MISSILE_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_DISPEL_EVIL_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_BOLT_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_HARM_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_CHILL_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_SHOCK_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM | TAR_FIGHT_VICT, TRUE, MAG_DAMAGE);
  spello(SPELL_CURE_WPN, 0, 0, 0, 0, 0, 0, 0,
	 POS_FIGHTING, TAR_CHAR_ROOM, FALSE, MAG_POINTS);
}
