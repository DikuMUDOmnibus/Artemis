...
if (victim == NULL || IS_NPC(victim) || obj != NULL)
    return;

  if (!IS_NPC(victim) && !PRF_FLAGGED(victim, PRF_SUMMONABLE) &&
      !PLR_FLAGGED(victim, PLR_KILLER) && ch != victim) {
    sprintf(buf, "%s just tried to recall you from: %s.\r\n"
    ...
    sprintf(buf, "You failed because %s has summon protection on.\r\n",
            GET_NAME(victim));
    send_to_char(buf, ch);
    sprintf(buf, "%s failed recalling %s from %s.",
...
ASPELL(spell_summon)
{
  ...
  if (!pk_allowed) {
    ...
    if (!IS_NPC(victim) && IS_SET(ROOM_FLAGS(ch->in_room), ROOM_PK_ALLOW) &&
        !PLR_FLAGGED(victim, PLR_KILLER)) {
      sprintf(buf, "%s just tried to summon you to: %s.\r\n"
	      "%s failed because the destination is a player killing room.\r\n"
              "To engage them in combat you must walk into the room of your own.\r\n",
	      GET_NAME(ch), world[ch->in_room].name,
              (ch->player.sex == SEX_MALE ? "He" : "She"));
      send_to_char(buf, victim);

      send_to_char("You failed because you are in a player killing room.\r\n", ch);

      sprintf(buf, "%s failed summoning %s to PK room %s.",
...
ASPELL(spell_charm)
{
  ...
  else if (IS_AFFECTED(victim, AFF_FARSEE))
    send_to_char("Your victim is already myopic!\r\n", ch);
...
ASPELL(spell_identify)
{
  ...
  if (obj) {
    ...
    send_to_char(buf, ch);
/* NEW */
  sprintf(buf, "Level Restriction: %d, Rent Restriction: %d\r\n", obj->obj_flags.lev_restrict, obj->obj_flags.rent_restrict);
  send_to_char(buf, ch);
/* END */
