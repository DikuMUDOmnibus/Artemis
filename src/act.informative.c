...
char *short_diag_char_to_char(struct char_data * i)
{
  static char buf[256];
  int percent;
  ...
  if (percent >= 100)
    strcpy(buf, "perfect condition");
  else if (percent >= 90)
    strcpy(buf, "starting to look tired");
  else if (percent >= 80)
    strcpy(buf, "quite a few scratches");
  else if (percent >= 70)
    strcpy(buf, "multiple contusions");
  else if (percent >= 60)
    strcpy(buf, "battered and mauled");
  else if (percent >= 50)
    strcpy(buf, "bleeding from many wounds");
  else if (percent >= 40)
    strcpy(buf, "severely injured");
  else if (percent >= 30)
    strcpy(buf, "many open wounds");
  else if (percent >= 20)
    strcpy(buf, "gaping wounds");
  else if (percent >= 10)
    strcpy(buf, "severely crippled");
  else if (percent >= 0)
    strcpy(buf, "terrible condition");
  else
    strcpy(buf, "bleeding, close to death");
...
void diag_char_to_char(struct char_data * i, struct char_data * ch)
{
  ...
  if (percent >= 100)
    strcat(buf, " appears to be in perfect condition.\r\n");
  else if (percent >= 90)
    strcat(buf, " is starting to look tired.\r\n");
  else if (percent >= 80)
    strcat(buf, " has quite a few scratches and bruises.\r\n");
  else if (percent >= 70)
    strcat(buf, " is oozing blood from multiple contusions.\r\n");
  else if (percent >= 60)
    strcat(buf, " is slightly battered and mauled.\r\n");
  else if (percent >= 50)
    strcat(buf, " is bleeding from many wounds.\r\n");
  else if (percent >= 40)
    strcat(buf, " is severely injured.\r\n");
  else if (percent >= 30)
    strcat(buf, " has many open wounds and is bleeding profusely.\r\n");
  else if (percent >= 20)
    strcat(buf, " has gaping wounds and several broken bones.\r\n");
  else if (percent >= 10)
    strcat(buf, " is severely crippled and likely to drop dead.\r\n");
  else if (percent >= 0)
    strcat(buf, " is in terrible condition!\r\n");
  else
    strcat(buf, " is bleeding, close to death.\r\n");
...
void look_in_direction(struct char_data * ch, int dir)
{
  ...
  if (EXIT(ch, dir)) {
    ...
    if (IS_SET(EXIT(ch, dir)->exit_info, EX_CLOSED) && EXIT(ch, dir)->keyword) {
      ...
      if((IS_AFFECTED(ch,AFF_FARSEE)) && (CAN_GO(ch,dir))){
        newroom=world[ch->in_room].dir_option[dir]->to_room;
...
ACMD(do_examine)
{
  ...
  if (tmp_object) {
	...
	(GET_OBJ_TYPE(tmp_object) == ITEM_CONTAINER)) {
      ...
      if(GET_OBJ_TYPE(tmp_object)==ITEM_FIREWEAPON){
...
ACMD(do_score)
{
  ...
  sprintf(buf,
	  ...
	  GET_MOVE(ch), GET_MAX_MOVE(ch));
          send_to_char(buf,ch);
  sprintf(buf, "Your armor class is %d/10, and your alignment is %d.\r\n",
  ...
  if (!IS_NPC(ch)) {
	...
	(titles[(int) GET_CLASS(ch)][GET_LEVEL(ch) + 1].exp) - GET_EXP(ch));
          send_to_char(buf,ch);
       } else if (GET_LEVEL(ch) <= 180) {
         p = EXP4LEV(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 220) {
         p = EXP4LEV1(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 245) {
         p = EXP4LEV2(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 265) {
         p = EXP4LEV3(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 278) {
         p = EXP4LEV4(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 289) {
         p = EXP4LEV5(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 299) {
         p = EXP4LEV6(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 
       else if (GET_LEVEL(ch) <= 300) {
         p = EXP4LEV7(GET_LEVEL(ch) + 1);
       sprintf(buf, "You need %d exp to reach your next level.\r\n", (p - GET_EXP(ch)));
          send_to_char(buf,ch);
       } 

    playing_time = real_time_passed((time(0) - ch->player.time.logson) +
  ...
  if (IS_AFFECTED(ch, AFF_FARSEE))
    strcat(buf, "Your eyes are myoptic.\r\n");

  if (IS_AFFECTED(ch, AFF_WATERWALK))
    strcat(buf, "Your toes have webbing between them.\r\n");
...
ACMD(do_users)
{
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    timeptr = asctime(localtime(&d->logsin_time));
    timeptr += 11;
    *(timeptr + 8) = '\0';
...
ACMD(do_levels)
{
  ...
  if (IS_NPC(ch)) {
    ...
    if(GET_LEVEL(ch) > LVL_IMMORT) {
      send_to_char("You have already reached immortality.\r\n", ch);
      return;
    }
  *buf = '\0';
  hi= GET_LEVEL(ch)+10;
  for (i = GET_LEVEL(ch); i < hi; i++) {
      if (i <= 100){
    ...
    sprintf(buf+strlen(buf), "%3d: %12d exp points\n\r",i, EXP4LEV(i));
  } else if (i <= 220){
    sprintf(buf+strlen(buf), "%3d: %12d exp points\n\r",i, EXP4LEV1(i));
  } else if (i <= 245){
    sprintf(buf+strlen(buf), "%3d: %12d exp points\n\r",i, EXP4LEV2(i));
...
ACMD(do_consider)
{
  ...
  if (diff <= -10)
    ...
    send_to_char("Bit Tough for you.\r\n", ch);
  else if (diff <= 20)
    send_to_char("You will die after 2 or 3 hits.\r\n", ch);
  else if (diff <= 30)
    send_to_char("You will die after 1 or 2 hits.\r\n", ch);
  else if (diff <= 40)
    send_to_char("You will die after 1 hit.\r\n", ch);
  else if (diff <= 50)
    send_to_char("You realy like to die don't you.\r\n", ch);
  else if (diff <= 60)
    send_to_char("Have you ever been killed by a little finger?\r\n", ch);
  else if (diff <= 70)
    send_to_char("You think that 7 of you might kill it.\r\n", ch);
  else if (diff <= 80)
    send_to_char("You think that 8 of you might kill it.\r\n", ch);
  else if (diff <= 90)
    send_to_char("You think that 9 of you might kill it.\r\n", ch);
  else if (diff <= 100)
    send_to_char("You think that 10 of you might kill it.\r\n", ch);
  else if (diff <= 110)
    send_to_char("You think that 11 of you might kill it.\r\n", ch);
  else if (diff <= 120)
    send_to_char("You think that 12 of you might kill it.\r\n", ch);
  else if (diff <= 130)
    send_to_char("You think that 13 of you might kill it.\r\n", ch);
  else if (diff <= 130)
    send_to_char("Forget it. You will DIE no matter how hard you try!\r\n", ch);
