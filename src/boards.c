...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  ...
  {3098, LVL_IMMORT, LVL_IMMORT, LVL_JIMPL, "etc/board.immort", 0},
  {3097, LVL_IMMORT, LVL_FREEZE, LVL_JIMPL, "etc/board.freeze", 0},
  {3096, 0, 0, LVL_IMMORT, "etc/board.social", 0},
  {3087, LVL_IMMORT, LVL_IMMORT, LVL_JIMPL, "etc/board.quest", 0},
  {3088, LVL_IMMORT, LVL_IMMORT, LVL_JIMPL, "etc/board.builders", 0},
  {3089, LVL_IMMORT, LVL_IMMORT, LVL_JIMPL, "etc/board.relations", 0},
  {3090, 0, 0, LVL_GRGOD, "etc/board.clan", 0},
  {3091 , LVL_GOD, LVL_GOD, LVL_JIMPL, "etc/board.holy", 0}, };
...
SPECIAL(gen_board)
{
  ...
  if ((board_type = find_board(ch)) == -1) {
    logs("SYSERR:  degenerate board!  (what the hell...)");
    return 0;
  }
  if (cmd == CMD_WRITE) {
...
void Board_write_message(int board_type, struct char_data * ch, char *arg)
{
  ...
  if ((NEW_MSG_INDEX(board_type).slot_num = find_slot()) == -1) {
    ...
    logs("SYSERR: Board: failed to find empty slot on write.");
    return;
  }
  /* skip blanks */
  skip_spaces(&arg);
...
/*    for (i = num_of_msgs[board_type] - 1; i >= 0; i--) { */
	...
	logs("SYSERR: The board is fubar'd.");
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg)
{
  ...
  if (MSG_SLOTNUM(board_type, ind) < 0 ||
    ...
    logs("SYSERR: Board is screwed up.");
    return 1;
  }
...
int Board_remove_msg(int board_type, struct char_data * ch, char *arg)
{
  ...
  if (slot_num < 0 || slot_num >= INDEX_SIZE) {
    logs("SYSERR: The board is seriously screwed up.");
...
void Board_load_board(int board_type)
{
  ...
  if (num_of_msgs[board_type] < 1 || num_of_msgs[board_type] > MAX_BOARD_MESSAGES) {
    logs("SYSERR: Board file corrupt.  Resetting.");
  ...
  for (i = 0; i < num_of_msgs[board_type]; i++) {
    ...
    if (!(tmp1 = (char *) malloc(sizeof(char) * len1))) {
      logs("SYSERR: Error - malloc failed for board header");
      exit(1);
    }
	...
	logs("SYSERR: Out of slots booting board!  Resetting...");
	Board_reset_board(board_type);
	return;
      }
      if (!(tmp2 = (char *) malloc(sizeof(char) * len2))) {
	logs("SYSERR: malloc failed for board text");
	exit(1);
      }
