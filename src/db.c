...
int weaponboost[]={-2,-1,-1,0,0,0,0,0,1,1,2};
...
void logs_zone_error(int zone, int cmd_no, char *message);
...
void do_rent(struct char_data *ch,int cmd, char *arg)
{
  ...
  if (IS_NPC(ch))
    return;
  if(cmd){
    if(!IS_SET(world[ch->in_room].room_flags,ROOM_RENT)){
...
void boot_db(void)
{
  ...
  logs("Resetting the game time:");
  reset_time();

  logs("Reading news, credits, help, bground, info & motds.");
  file_to_string_alloc(NEWS_FILE, &news);
  file_to_string_alloc(CREDITS_FILE, &credits);
  file_to_string_alloc(MOTD_FILE, &motd);
  file_to_string_alloc(IMOTD_FILE, &imotd);
  ...
  logs("Opening help file.");
  if (!(help_fl = fopen(HELP_KWRD_FILE, "r")))
    logs("   Could not open help file.");
  else
    help_index = build_help_index(help_fl, &top_of_helpt);

  boot_world();

  logs("Generating player index.");
  build_player_index();

  logs("Loading fight messages.");
  load_messages();

  logs("Loading social messages.");
  boot_social_messages();

  logs("Assigning function pointers:");
  ...
  logs("Sorting command list and spells.");
  sort_commands();
  sort_spells();

  logs("Booting mail system.");
  if (!scan_file()) {
    logs("    Mail boot failed -- Mail system disabled");
    no_mail = 1;
  }
  logs("Reading banned site and invalid-name list.");
  load_banned();
  Read_Invalid_List();

  if (!no_rent_check) {
    logs("Deleting timed-out crash and rent files:");
    update_obj_file();
    logs("Done.");
  }
...
void build_player_index(void)
{
  ...
  if (!(player_fl = fopen(PLAYER_FILE, "r+b"))) {
    if (errno != ENOENT) {
      ...
      logs("No playerfile.  Creating a new one.");
      touch(PLAYER_FILE);
      if (!(player_fl = fopen(PLAYER_FILE, "r+b"))) {
	perror("fatal error opening playerfile");
	exit(1);
...
void check_start_rooms(void)
{
  ...
  if ((r_mortal_start_room = real_room(mortal_start_room)) < 0) {
    logs("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_immort_start_room = real_room(immort_start_room)) < 0) {
    if (!mini_mud)
      logs("SYSERR:  Warning: Immort start room does not exist.  Change in config.c.");
    r_immort_start_room = r_mortal_start_room;
  }
  if ((r_frozen_start_room = real_room(frozen_start_room)) < 0) {
    if (!mini_mud)
      logs("SYSERR:  Warning: Frozen start room does not exist.  Change in config.c.");
...
void renum_zone_table(void)
{
  ...
  for (zone = 0; zone <= top_of_zone_table; zone++)
	...
	b = ZCMD.arg2 = real_object(ZCMD.arg2);
      ...
      if (a < 0 || b < 0) {
	if (!mini_mud)
	  logs_zone_error(zone, cmd_no, "Invalid vnum, cmd disabled");
	ZCMD.command = '*';
      }
    }
}
...
for (k = 0; k < MAX_OBJ_AFFECT; k++) {
    obj_proto[i].spec_hit[j].spell_num = 0;
    obj_proto[i].spec_hit[j].spell_lev = 0;
    obj_proto[i].spec_hit[j].chance = 0;
  }

  for (l = 0; l < MAX_OBJ_AFFECT; l++) {
    obj_proto[i].spec_worn[l].spell_num = 0;
    obj_proto[i].spec_worn[l].spell_lev = 0;
  }

  for (m = 0; m < MAX_OBJ_AFFECT; m++) {
    obj_proto[i].spec_curse[m].spell_num = 0;
    obj_proto[i].spec_curse[m].spell_lev = 0;
    obj_proto[i].spec_curse[m].chance = 0;
  }
 ...
 k = 0;
  ...
  m = 0;
/* NEW */
  obj_proto[i].obj_flags.lev_restrict = 0;
  obj_proto[i].obj_flags.rent_restrict = 0;
/* END */
...
/* NEW */
    case 'R':
      if (!get_line(obj_f, line)) {
        fprintf(stderr, "Missing neumerics after RESTRICTION spec in %s.\n", buf2);
      }
      if ((retval = sscanf(line," %d %d ", t, t + 1)) != 2)
        fprintf(stderr, "Error in RESTRICTION spec (%d fields, expected 2) in %s\n", retval, buf2);
      else
        obj_proto[i].obj_flags.lev_restrict = t[0];
        obj_proto[i].obj_flags.rent_restrict = t[1];
      break;
/* END */
 ...
 case 'B':
      if (k >= MAX_OBJ_AFFECT) {
        fprintf(stderr, "Too many B fields (%d max), %s\n", MAX_OBJ_AFFECT, buf2
);
        exit(1);
      }
      get_line(obj_f, line);
      sscanf(line, " %d %d %d ", t, t + 1, t + 2);
      obj_proto[i].spec_hit[k].spell_num = t[0];
      obj_proto[i].spec_hit[k].spell_lev = t[1];
      obj_proto[i].spec_hit[k].chance = t[2];
      k++;
      break;
    case 'C':
      if (l >= MAX_OBJ_AFFECT) {
        fprintf(stderr, "Too many C fields (%d max), %s\n", MAX_OBJ_AFFECT, buf2
);
        exit(1);
      }
      get_line(obj_f, line);
      sscanf(line, " %d %d ", t, t + 1);
      obj_proto[i].spec_worn[l].spell_num = t[0];
      obj_proto[i].spec_worn[l].spell_lev = t[1];
      l++;
      break;
    case 'D':
      if (m >= MAX_OBJ_AFFECT) {
        fprintf(stderr, "Too many D fields (%d max), %s\n", MAX_OBJ_AFFECT, buf2
);
        exit(1);
      }
      get_line(obj_f, line);
      sscanf(line, " %d %d %d ", t, t + 1, t + 2);
      obj_proto[i].spec_curse[m].spell_num = t[0];
      obj_proto[i].spec_curse[m].spell_lev = t[1];
      obj_proto[i].spec_curse[m].chance = t[2];
      m++;
      break;
    case '$':
    case '#':
...
struct obj_data *read_object(int nr, int type)
{
  ...
  if (nr < 0) {
    logs("SYSERR: trying to create obj with negative num!");
    return NULL;
  }
  if (type == VIRTUAL) {
...
#define ZONE_ERROR(message) \
	{ logs_zone_error(zone, cmd_no, message); last_cmd = 0; }

/* execute the reset command table of a given zone */
void reset_zone(int zone)
{
  int cmd_no, last_cmd = 0, found = 0, given = 0, prev_mob_rnum;
  struct char_data *mob;
  struct obj_data *obj, *obj_to, *tmp_obj;
...
*/
    ...
    case 'P':			/* object to object */
	  ...
	  for (k = object_list; k; k = k->next) {
            given = 0;
            found = 0;
            if ((k->item_number == ZCMD.arg3) && 
		(GET_OBJ_TYPE(k) == ITEM_CONTAINER)) {
              for (tmp_obj = k->contains; tmp_obj; tmp_obj = tmp_obj->next_content)
                if (tmp_obj->item_number == ZCMD.arg1) {
                  found = 1;
                }
              if (!found) {
                obj = read_object(ZCMD.arg1, REAL);
	  ...
	  obj_to_char(obj, mob);
        ...
        if (obj_index[ZCMD.arg1].number < ZCMD.arg2) {
          for (i = character_list; i; i = i->next) {
            given = 0;
            found = 0;
            if ((i->nr == prev_mob_rnum) && (IS_NPC(i))) {
	  ...
	  if (ZCMD.arg3 < 0 || ZCMD.arg3 >= NUM_WEARS) {
        ...
        if (obj_index[ZCMD.arg1].number < ZCMD.arg2) {
          for (i = character_list; i; i = i->next) {
            ...
            found = 0;
            if ((i->nr == prev_mob_rnum) && (IS_NPC(i))) {
              if (GET_EQ(i, ZCMD.arg3))
                found = 1;
              if (!found) {
                obj = read_object(ZCMD.arg1, REAL);
                equip_char(i, obj, ZCMD.arg3);
                if(IS_SET(obj->obj_flags.extra_flags,ITEM_VARIABLE)){
                  switch(GET_OBJ_TYPE(obj)){
                  case ITEM_ARMOR:
                    if(obj->affected[0].location){
...
/*   affect_total(ch); also - unnecessary?? */
  ...
  if (!IS_AFFECTED(ch, AFF_POISON) &&
      (((long) (time(0) - st->last_logson)) >= SECS_PER_REAL_HOUR)) {
...
void char_to_store(struct char_data * ch, struct char_file_u * st)
{
   ...
   * effects are doubled when the char logss back in.
   */
  ...
  if ((i >= MAX_AFFECT) && af && af->next)
    logs("SYSERR: WARNING: OUT OF STORE ROOM FOR AFFECTED TYPES!!!");

  ch->aff_abils = ch->real_abils;

  st->birth = ch->player.time.birth;
  st->played = ch->player.time.played;
  st->played += (long) (time(0) - ch->player.time.logson);
  st->last_logson = time(0);
...
void free_char(struct char_data * ch)
{
  ...
  if (ch->player_specials != NULL && ch->player_specials != &dummy_mob) {
    ...
    if (IS_NPC(ch))
      logs("SYSERR: Mob had player_specials allocated!");
  }
...
int mprog_name_to_type (char *name)
{
 ...
 char *fread_word(FILE *fp)
 {
     ...
     logs("SYSERR: Fread_word: word too long.");
     exit(1);
     return NULL;
 }
                             ...
                             struct index_data *pMobIndex)
 {
   ...
   switch (letter = fread_letter(progfile))
   {
     case '>':
      break;
     case '|':
        logs("empty mobprog file.");
        exit(1);
      break;
     default:
        logs("in mobprog file syntax error.");
        exit(1);
      break;
   }
   ...
   while (!done)
   {
     ...
     {
      case ERROR_PROG:
         logs("mobprog file type error");
         exit(1);
       break;
      case IN_FILE_PROG:
         logs("mprog file contains a call to file.");
         exit(1);
       break;
      default:
