...
ACMD(do_quit)
{
 ...
 if (IS_NPC(ch) || !ch->desc)
  ...
  if (subcmd != SCMD_QUIT && GET_LEVEL(ch) < LVL_IMMORT)
    ...
    mudlogs(buf, NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE);
...
ACMD(do_use)
{
  ...
  if (!mag_item || !isname(arg, mag_item->name)) {
    ...
    case SCMD_USE:
      ...
      break;
    default:
      logs("SYSERR: Unknown subcmd passed to do_use");
      return;
      break;
    }
  }
...
ACMD(do_display)
{
  ...
  if (!*argument) {
    send_to_char("Usage: prompt { H | M | V | T | E | all | none }\r\n", ch);
    return;
  }
  if ((!str_cmp(argument, "on")) || (!str_cmp(argument, "all")))
    SET_BIT(PRF_FLAGS(ch), PRF_DISPHP | PRF_DISPMANA | PRF_DISPMOVE | PRF_DISPTANK | PRF_DISPENEMY);
  else {
    REMOVE_BIT(PRF_FLAGS(ch), PRF_DISPHP | PRF_DISPMANA | PRF_DISPMOVE | PRF_DISPTANK | PRF_DISPENEMY);
...
ACMD(do_gen_write)
{
  ...
  mudlogs(buf, CMP, LVL_IMMORT, FALSE);
...
ACMD(do_gen_tog)
{
  ...
  case SCMD_AUTOEXIT:
    ...
    break;
  default:
    logs("SYSERR: Unknown subcmd in do_gen_toggle");
    return;
    break;
  }
