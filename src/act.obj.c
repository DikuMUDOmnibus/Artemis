		      ...
		      struct obj_data * cont)
{
  if (GET_OBJ_WEIGHT(cont) + GET_OBJ_WEIGHT(obj) > GET_OBJ_VAL(cont, 0))
    ...
    if (GET_LEVEL(ch) >= LVL_IMMORT) {
      sprintf(buf, "%s has put %s in %s.", GET_NAME(ch), 
...
int can_take_obj(struct char_data * ch, struct obj_data * obj)
{
  if (GET_LEVEL(ch) >= LVL_GOD) {
    ...
    return 0;
  } else if (obj->obj_flags.rent_restrict > GET_LEVEL(ch)) {
    act("$p: you can't take that at your level!", FALSE, ch, obj, 0, TO_CHAR);
    return 0;
  }
				     ...
				     struct obj_data * cont, int mode)
{
  if (mode == FIND_OBJ_INV || can_take_obj(ch, obj)) {
    if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
      ...
      obj_to_char(obj, ch);
      if (GET_LEVEL(ch) >= LVL_IMMORT) {
        sprintf(buf, "%s has gotten %s from %s.", GET_NAME(ch),
          obj->short_description, cont->short_description);
        logs(buf);
      }
		            ...
		            byte mode, sh_int RDR)
{
  ...
  if (amount <= 0)
    ...
    if (mode != SCMD_JUNK) {
      ...
      if (mode == SCMD_DONATE) {
        if (GET_LEVEL(ch) >= LVL_IMMORT) {
          sprintf(buf, "%s has donated %d gold coins.", GET_NAME(ch), amount);
          logs(buf);
        }
	...
	act("$p suddenly appears in a puff of orange smoke!", 0, 0, obj, 0, TO_ROOM);
        ...
        if (GET_LEVEL(ch) >= LVL_IMMORT) {
          sprintf(buf, "%s has dropped %d gold coins.", GET_NAME(ch), amount);
          logs(buf);
        }
		     ...
		     byte mode, char *sname, sh_int RDR)
{
  ...
  case SCMD_DROP:
    if (GET_LEVEL(ch) >= LVL_IMMORT) {
      sprintf(buf, "%s has dropped %s in %s.", GET_NAME(ch),
        obj->short_description, world[ch->in_room].name);
      logs(buf);
    }
    obj_to_room(obj, ch->in_room);
  ...
  case SCMD_DONATE:
    if (GET_LEVEL(ch) >= LVL_IMMORT) {
      sprintf(buf, "%s has donated %s.", GET_NAME(ch),
  ...
  case SCMD_JUNK:
    ...
    if (GET_LEVEL(ch) >= LVL_IMMORT) {
      sprintf(buf, "%s has junked %s.", GET_NAME(ch),
        obj->short_description);
      logs(buf);
    }
    extract_obj(obj);
    ...
    break;
  default:
    logs("SYSERR: Incorrect argument passed to perform_drop");
    break;
  }
		       ...
		       struct obj_data * obj)
{
  ...
  if (obj->obj_flags.rent_restrict > GET_LEVEL(vict)) {
    act("$E can't take that at $S level!", FALSE, ch, obj, vict, TO_CHAR);
    return;
  }
  obj_from_char(obj);
  obj_to_char(obj, vict);
  if (GET_LEVEL(ch) >= LVL_IMMORT) {
    sprintf(buf, "%s has given %s to %s.", GET_NAME(ch),
		            ...
		            int amount)
{
  ...
  if (GET_LEVEL(ch) >= LVL_IMMORT) {
    sprintf(buf, "%s has given %d gold coins to %s.", GET_NAME(ch),
...
void do_reload(struct char_data *ch, char *argument, int cmd)
{
  ...
  if (*arg1) {
    if (*arg2) {
      ...
      if (gun) {
        if(GET_OBJ_TYPE(gun) != ITEM_FIREWEAPON){
...
void weight_change_object(struct obj_data * obj, int weight)
{
  ...
  if (obj->in_room != NOWHERE) {
    ...
    logs("SYSERR: Unknown attempt to subtract weight from an object.");
  }
}
...
/* NEW */
  if (obj->obj_flags.lev_restrict > GET_LEVEL(ch)) {
    act("You cant seem to figure out how to use $p.", FALSE, ch, obj, 0, TO_CHAR);
    return;
  }
/* END */
...
void perform_remove(struct char_data * ch, int pos)
{
  ...
  if (!(obj = GET_EQ(ch, pos))) {
    logs("Error in perform_remove: bad pos passed.");
    return;
  }
  if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
    act("$p: you can't carry that many items!", FALSE, ch, obj, 0, TO_CHAR);
