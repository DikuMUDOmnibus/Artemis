...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check)
{
  ...
  if(((n=GET_COND(ch,DRUNK)) > 1) && number(0,n)){
    dir=number(0,4);
    if(!CAN_GO(ch,dir)){
      act("$n staggers and almosts falls down.", TRUE, ch, 0,0, TO_ROOM);
      send_to_char("You bump into something, you drunk!\n\r",ch);
      return 0;
    }
    }
...
int ok_pick(struct char_data *ch, int keynum, int pickproof, int scmd)
{
  ...
  if (scmd == SCMD_PICK) {
    if ((keynum < 0) && (GET_LEVEL(ch)<LVL_GOD))
      send_to_char("Odd - you can't seem to find a keyhole.\r\n", ch);
    else if((pickproof) && (GET_LEVEL(ch)<LVL_GOD))
...
ACMD(do_follow)
{
  ...
  if (IS_AFFECTED(ch, AFF_CHARM) && (ch->master)) {
    ...
    if ((GET_LEVEL(leader) < 40) || (GET_LEVEL(ch) < 40)) {
      if (GET_LEVEL(leader) >= GET_LEVEL(ch)+11) {
         act(" Sorry, but you can't follow someone more than 10 levels above you.", FALSE, ch, 0, 0, TO_CHAR);
         return;
      }
      if ((GET_LEVEL(ch) < LVL_IMMORT) && (GET_LEVEL(leader)+11 <= GET_LEVEL(ch))) {
         act(" Sorry, but you can't follow someone more than 10 levels below you.", FALSE, ch, 0, 0, TO_CHAR);
         return;
      }
    }
    if ((GET_LEVEL(leader) < 100) || (GET_LEVEL(ch) < 100)) {
      if (GET_LEVEL(leader) >= GET_LEVEL(ch) + 26) {
	act("Sorry, but you can't follow someone more than 25 levels above you.",
		FALSE, ch, 0, 0, TO_CHAR);
	return;
      }
      if ((GET_LEVEL(ch) < LVL_IMMORT) && (GET_LEVEL(leader) + 26 <= GET_LEVEL(ch))) {
	act("Sorry, but you can't follow someone more than 25 levels below you.",
		FALSE, ch, 0, 0, TO_CHAR);
	return;
      }
    }
