...
int main(int argc, char **argv)
{
  ...
  while ((pos < argc) && (*(argv[pos]) == '-')) {
	...
	logs("Directory arg expected after option -d.");
	exit(1);
      }
      break;
    case 'm':
      mini_mud = 1;
      no_rent_check = 1;
      logs("Running in minimized mode & with no rent check.");
      break;
    case 'c':
      scheck = 1;
      logs("Syntax check mode enabled.");
      break;
    case 'q':
      no_rent_check = 1;
      logs("Quick boot mode -- rent check supressed.");
      break;
    case 'r':
      restrict = 1;
      logs("Restricting game -- no new players allowed.");
      break;
    case 'R':
      restrict = LVL_IMPL+1;
      logs("Close game, no players allowed, port message only.");
      break;
    case 's':
      no_specials = 1;
      logs("Suppressing assignment of special routines.");
      break;
    default:
...
void init_game(int port)
{
  ...
  logs("Opening mother connection.");
  ...
  logs("Signal trapping.");
  signal_setup();

  logs("Entering game loop.");

  game_loop(mother_desc);

  logs("Closing all sockets.");
  ...
  logs("Normal termination of game.");
}
...
#define OPEN_MAX FOPEN_MAX
  ...
  if (max_descs <= 0) {
    logs("Non-positive max player limit!");
    exit(1);
  }
...
void game_loop(int mother_desc)
{
  ...
  char comm[MAX_INPUT_LENGTH], inputswap[MAX_INPUT_LENGTH];
  struct descriptor_data *d, *next_d;
  int pulse = 0, mins_since_crashsave = 0, maxdesc, aliased, spamcount = 0;
  ...
  while (!circle_shutdown) {
    ...
    if (descriptor_list == NULL) {
      logs("No connections.  Going to sleep.");
      FD_ZERO(&input_set);
      FD_SET(mother_desc, &input_set);
      if (select(mother_desc + 1, &input_set, (fd_set *) 0, (fd_set *) 0, NULL) < 0) {
	if (errno == EINTR)
	  logs("Waking up to process signal.");
	else
	  perror("Select coma");
      } else
	logs("New connection.  Waking up.");
      gettimeofday(&last_time, (struct timezone *) 0);
    }
    ...
    /* Code to get rid of Snapper-Heads who spam the mud */
/*	if ((--(d->wait) <= 0) && strcmp(d->inputswap, d->input))
	{	
		spamcount = 0;
		strcpy (d->input, d->inputswap);
	}
	else
	if (++spamcount >= 21)
	{
		act("SPAMMING IS NOT ALLOWED.", TRUE, d->character, 0, 0, TO_ROOM);
		close_socket(d);
	}  */
    ...
    if (FIGHTING(d->character)) {
      if (FIGHTING(FIGHTING(d->character))) {
        if (PRF_FLAGGED(d->character, PRF_DISPTANK)) 
          sprintf(prompt, "<T: %s, TC: %s>\r\n", 
          GET_NAME(FIGHTING(FIGHTING(d->character))),
          short_diag_char_to_char(FIGHTING(FIGHTING(d->character))));
      }
      if (PRF_FLAGGED(d->character, PRF_DISPENEMY))
        sprintf(prompt, "%s<E: %s, EC: %s>\r\n", prompt,
        GET_NAME(FIGHTING(d->character)),
        short_diag_char_to_char(FIGHTING(d->character)));
    }
...
int new_descriptor(int s)
{
  ...
  /* find, and store, the IP address. */
  addr = ntohl(peer.sin_addr.s_addr);
  newd->ip[0] = ((addr & 0xFF000000) >> 24);
  newd->ip[1] = ((addr & 0x00FF0000) >> 16);
  newd->ip[2] = ((addr & 0x0000FF00) >> 8);
  newd->ip[3] = ((addr & 0x000000FF));
  /* find the sitename */
  ...
  /* All done in the nanny - for now. */
#if 0
  if (isbanned(newd->host) == BAN_ALL) {
    close(desc);
    sprintf(buf2, "Connection attempt denied from [%s]", newd->host);
    mudlogs(buf2, CMP, LVL_GOD, TRUE);
    free(newd);
    return 0;
  }
#endif
  ...
  mudlogs(buf2, CMP, LVL_GOD, FALSE);
#endif
...
#endif
      if (errno == EAGAIN)
	logs("process_output: socket write would block, about to close");
      else
...
int process_input(struct descriptor_data * t)
{
  ...
  do {
    if (space_left <= 0) {
      logs("process_input: about to close connection: input overflow");
      return -1;
    }
	...
	return 0;		/* the read would have blocked: just means no
      ...
      logs("EOF on socket read (connection broken by peer)");
      return -1;
    }
...
void close_socket(struct descriptor_data * d)
{
  ...
  if (d->character) {
    ...
    if (d->connected == CON_PLAYING) {
      ...
      if (!d->nolog)
        mudlogs(buf, CMP, LVL_IMMORT, TRUE);
      free_char(d->character);
    }
  } else
    mudlogs("Losing descriptor without char.", CMP, LVL_IMMORT, TRUE);
...
void check_idle_passwords(void)
{
  ...
  /* traverse the linked list of descriptors and check for idling people */
...
{
  if (!tics) {
    logs("SYSERR: CHECKPOINT shutdown: tics not updated");
    abort();
  } else
    tics = 0;
}
...
void reread_wizlists()
{
  ...
  mudlogs("Signal received - rereading wizlists.", CMP, LVL_IMMORT, TRUE);
  reboot_wizlists();
}
...
void unrestrict_game()
{
  ...
  mudlogs("Received SIGUSR2 - completely unrestricting game (emergent)",
	 BRF, LVL_IMMORT, TRUE);
  ban_list = NULL;
  restrict = 0;
  num_invalid = 0;
...
void hupsig()
{
  logs("Received SIGHUP, SIGINT, or SIGTERM.  Shutting down...");
		      ...
		      void *vict_obj, struct char_data * to)
{
  ...
  for (;;) {
	...
	break;
      default:
	logs("SYSERR: Illegal $-code to act():");
	strcpy(buf1, "SYSERR: ");
	strcat(buf1, orig);
	logs(buf1);
	break;
	      ...
	      struct obj_data * obj, void *vict_obj, int type)
{
  ...
  if (ch && ch->in_room != NOWHERE)
    ...
    logs("SYSERR: no valid target to act()!");
    return;
  }
