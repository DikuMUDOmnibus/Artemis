...
ACMD(do_flick)
{
  ...
  if (!(obj = get_obj_in_list_vis(victim, obj_name, victim->carrying))) {
    ...
    if (!obj) {
      ...
      send_to_char("Done.\n\r", ch);
      sprintf(buf,"(GC) %s has flicked %s off %s",GET_NAME(ch), 
GET_NAME(victim), obj_name);
      mudlogs( buf, NRM, MAX(LVL_GOD,GET_INVIS_LEV(ch)), TRUE);
    }
...
ACMD(do_goto)
{
  ...
  if (POOFOUT(ch))
    ...
    strcpy(buf, "$n gestures. A portal appears, and $n steps through.");
  ...
  if (POOFIN(ch))
    ...
    strcpy(buf, "$n appears.  You feel the need to Grovel in solem worship.");
...
ACMD(do_trans)
{
  ...
  if (!*buf)
	...
	return;
      ...
      sprintf(buf, "%s has transported %s to %s.\r\n", GET_NAME(ch),
...
ACMD(do_teleport)
{
  ...
  if (!*buf)
    ...
    sprintf(buf, "%s has teleported %s to %s.\r\n", GET_NAME(ch), 
		GET_NAME(victim), world[target].name);
    logs(buf);
    look_at_room(victim, 0);
  }
}
...
void do_stat_object(struct char_data * ch, struct obj_data * j)
{
  ...
  send_to_char(buf, ch);
/* NEW */
  sprintf(buf, "Level Restriction: %d, Rent Restriction: %d\r\n", j->obj_flags.lev_restrict, j->obj_flags.rent_restrict);
  send_to_char(buf, ch);
/* END */
  ...
  case ITEM_WEAPON:
     if(IS_SET(j->obj_flags.extra_flags,ITEM_SFX)){
         sprintf(buf, "SFX Type: %d, Todam: %dd%d, Type: %d", GET_OBJ_VAL(j, 0),
	    GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2), GET_OBJ_VAL(j, 3));
    }
    else {
...
void do_stat_character(struct char_data * ch, struct char_data * k)
{
  ...
  if (!IS_NPC(k)) {
    ...
    strcpy(buf2, (char *) asctime(localtime(&(k->player.time.logson))));
    buf1[10] = buf2[10] = '\0';
...
ACMD(do_shutdown)
{
  extern int circle_shutdown, circle_reboot, shutdownwarn;
...
ACMD(do_switch)
{
  ...
  if (ch->desc->original)
    ...
    send_to_char(OK, ch);
    sprintf(buf, "(GC) %s has switched into %s", GET_NAME(ch), 
GET_NAME(victim));
    mudlogs(buf, NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
...
ACMD(do_load)
{
  ...
  if (!strn_cmp(argument, " ban", 4) || !strn_cmp(argument, "ban", 3)) {
    send_to_char("Reloading the banned list, please wait.\r\n", ch);
    reload_banned();
    send_to_char("Done.\r\n", ch);
    return;
  }
...
GET_NAME(mob));
    ...
    act("$n has created $p!", FALSE, ch, obj, 0, TO_ROOM);
       ...
       act("You snare $p after you create it.", FALSE, ch, obj, 0, TO_CHAR);
  } else
...
ACMD(do_advance)
{
  ...
  void set_level(struct char_data * ch, int level);
  ...
  if ((GET_LEVEL(ch) <= GET_LEVEL(victim) && GET_NAME(ch) != "Cat")) {
  ...
  if (newlevel <= 100)
   ...
   {
     /* so they get the hp/mana increases */
     if (GET_LEVEL(victim) > 100)
    gain_exp_regardless(victim,(EXP4LEV(GET_LEVEL(victim)) - GET_EXP(victim)));
     set_level(victim, newlevel);
   }
...
ACMD(do_last)
{
  ...
  if ((chdata.level > GET_LEVEL(ch)) && (GET_LEVEL(ch) < LVL_JIMPL)) {
...
ACMD(do_zreset)
{
  ...
  if (*arg == '*') {
    ...
    mudlogs(buf, NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE);
    return;
  } else if (*arg == '.')
...
ACMD(do_wizutil)
{
  ...
  if (!*arg)
	...
	return;
      ...
      mudlogs(buf, BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
      break;
    case SCMD_NOTITLE:
	...
	return;
      ...
      break;
    default:
      logs("SYSERR: Unknown subcmd passed to do_wizutil (act.wizard.c)");
      break;
    }
...
ACMD(do_show)
{
  ...
  case 2:			/* player */
    ...
    sprintf(buf,
	    ...
	    buf, birth, ctime(&vbuf.last_logson), (int) (vbuf.played / 3600),
...
ACMD(do_set)
{
  ...
  struct set_struct {
   ...
   { "gold",		LVL_GRGOD, 	BOTH, 	NUMBER },
   { "bank",		LVL_GRGOD, 	PC, 	NUMBER },  /* 20 */
   { "exp", 		LVL_JIMPL, 	BOTH, 	NUMBER },
   { "hitroll", 	LVL_GRGOD, 	BOTH, 	NUMBER },
   { "damroll", 	LVL_GRGOD, 	BOTH, 	NUMBER },
   { "invis",		LVL_JIMPL, 	PC, 	NUMBER },
   ...
   { "level",		LVL_JIMPL, 	BOTH, 	NUMBER },
   { "room",		LVL_JIMPL, 	BOTH, 	NUMBER },  /* 35 */
   { "roomflag", 	LVL_GRGOD, 	PC, 	BINARY },
   { "siteok",		LVL_GRGOD, 	PC, 	BINARY },
   { "deleted", 	LVL_JIMPL, 	PC, 	BINARY },
  ...
  case 3:
    ...
    break;
  case 4:
    vict->points.max_hit = RANGE(-1500000, 1500000);
    affect_total(vict);
    break;
  case 5:
    vict->points.max_mana = RANGE(-1500000, 1500000);
    affect_total(vict);
   break;
  case 6:
    vict->points.max_move = RANGE(-1500000, 1500000);
    affect_total(vict);
    break;
  case 7:
    ...
    break;
  case 8:
    vict->points.mana = RANGE(-9, vict->points.max_mana);
    affect_total(vict);
    break;
  case 9:
    vict->points.move = RANGE(-9, vict->points.max_move);
    affect_total(vict);
    break;
  case 10:
  ...
  case 17:
    ...
    break;
  case 18:
    vict->points.armor = RANGE(-1000, 1000);
    affect_total(vict);
    break;
  case 19:
  ...
  case 21:
    ...
    break;
  case 22:
    vict->points.hitroll = RANGE(-20, 9999);
    affect_total(vict);
    break;
  case 23:
    vict->points.damroll = RANGE(-20, 9999);
    affect_total(vict);
    break;
  case 24:
    if (GET_LEVEL(ch) < LVL_JIMPL && ch != vict) {
  ...
  case 34:
    ...
    if((GET_LEVEL(ch) < (int) value) && (GET_LEVEL(ch) < LVL_IMPL)){
      send_to_char("You can't do that.\r\n", ch);
      return;
    }
    RANGE(0, 999);
  ...
  case 39:
    if ((i = parse_class(*val_arg)) == CLASS_UNDEFINED) {
      if (IS_NPC(vict)) {
        send_to_char("Setting monster class to 'normal'.\r\n", ch);
        GET_CLASS(vict) = -1;
      } else
  ...
  if (fields[l].type == BINARY) {
    sprintf(buf, "%s %s for %s.\r\n", fields[l].cmd, ONOFF(on),
	    GET_NAME(vict));
    CAP(buf);
    sprintf(buf1, "(GC) %s has set %s %s for %s", 
GET_NAME(ch),fields[l].cmd, ONOFF(on), GET_NAME(vict));
    mudlogs(buf1, NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)),TRUE);
  } else if (fields[l].type == NUMBER) {
    sprintf(buf, "%s's %s set to %d.\r\n", GET_NAME(vict),
	    fields[l].cmd, value);
    sprintf(buf1, "(GC) %s has set %s's %s to %d", GET_NAME(ch), 
...
ACMD(do_syslogs)
{
  ...
  if (!*arg) {
    ...
    sprintf(buf, "Your syslogs is currently %s.\r\n", logstypes[tp]);
    send_to_char(buf, ch);
    return;
  }
  if (((tp = search_block(arg, logstypes, FALSE)) == -1)) {
    send_to_char("Usage: syslogs { Off | Brief | Normal | Complete }\r\n", ch);
    return;
  }
  REMOVE_BIT(PRF_FLAGS(ch), PRF_LOG1 | PRF_LOG2);
  SET_BIT(PRF_FLAGS(ch), (PRF_LOG1 * (tp & 1)) | (PRF_LOG2 * (tp & 2) >> 1));

  sprintf(buf, "Your syslogs is now %s.\r\n", logstypes[tp]);
  send_to_char(buf, ch);
}
