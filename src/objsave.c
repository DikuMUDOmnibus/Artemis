...
int Crash_is_unrentable(struct obj_data * obj,struct char_data * ch)
{
  if (!obj)
    return 0;
  ...
  if (IS_OBJ_STAT(obj, ITEM_NORENT) || GET_OBJ_RENT(obj) < 0 ||
	...
	obj->obj_flags.rent_restrict > GET_LEVEL(ch)) 
    return 1;
...
void Crash_extract_norents(struct obj_data * obj, struct char_data * ch)
{
  if (obj) {
    Crash_extract_norents(obj->contains,ch);
    Crash_extract_norents(obj->next_content,ch);
    if (Crash_is_unrentable(obj,ch))
      extract_obj(obj);
  }
}
