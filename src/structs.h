...
#define ROOM_RENT               (1 << 16)  /* Players can rent in room  */
#define ROOM_PK_ALLOW		(1 << 17)  /* Players can pk in room..  */
...
#define MOB_SMART        (1 << 18) /* Mob attacks weakest character in room */
#define MOB_NOXP         (1 << 19) /* No exp gained when mob_noxp is killed */ 
...
#define PRF_DISPTANK    (1 << 22)
#define PRF_DISPENEMY   (1 << 23)
...
#define AFF_FARSEE	      (1 << 17)	   /* Char can see into next room */
...
#define CON_DC_PASSWORD	 18		/* Password to DC char. */
#define CON_GETSTATS	 19
...
#define ITEM_SFX           (1 << 17)
#define ITEM_TAKE_LEVEL    (1 << 18)
...
#define LVL_IMPL	350
#define LVL_JIMPL       340
#define LVL_GRGOD	330
#define LVL_GOD		320
#define LVL_IMMORT	301
...
/* NEW */
   int  lev_restrict;   /* level restriction on object      */
   int  rent_restrict;  /* rent restriction on object       */
/* END */
...
/* Used in obj_file_elem *NO IDEA ?!?* */
struct obj_weapon_aff {
   int spell_num;
   int spell_lev;
   int chance;
};
/* Used in obj_file_elem *NO IDEA ?!?* */
struct obj_armor_aff {
   int spell_num;
   int spell_lev;
};
...
struct obj_weapon_aff spec_hit[MAX_OBJ_AFFECT];  /* wielded aff. */
struct obj_armor_aff spec_worn[MAX_OBJ_AFFECT];      /* worn aff. */
   struct obj_weapon_aff spec_curse[MAX_OBJ_AFFECT];  /* wielded bad */
...
struct time_data {
   ...
   time_t logson;    /* Time of the last logson (used to calculate played) */
 ...
 * playerfile into memory when players logs in.
 */
struct player_special_data_saved {
   ...
   short freeze_level;		/* Level of god who froze char, if any	*/
...
struct player_special_data {
   ...
   char **notes;		/* notes on a given player. Array of lines */
   int num_notes;		/* how many notes the player has	*/
};
...
struct char_file_u {
   ...
   time_t last_logson;		/* Time (in secs) of last logson */
   char host[HOST_LENGTH+1];	/* host of last logson */
};
/* ====================================================================== */
...
struct descriptor_data {
   ...
   int idle_tics;
   byte	bad_pws;		/* number of bad pw attemps this logsin	*/
   byte nolog;			/* don't log the losing connection bit	*/
   int	connected;		/* mode of 'connectedness'		*/
   int	wait;			/* wait for how many loops		*/
   int	desc_num;		/* unique num assigned to desc		*/
   time_t logsin_time;		/* when the person connected		*/
   ...
   struct char_data *to_dc;	/* char to dc (for CON_DC_PASSWORD)	*/
