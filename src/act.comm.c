...
ACMD(do_gsay)
{
  ...
  if (PLR_FLAGGED(ch, PLR_NOSHOUT)) {
    send_to_char("You cannot group-say!\r\n", ch);
    return;
  }
...
{
  if (PLR_FLAGGED(ch, PLR_NOSHOUT)) {
    send_to_char("The words die on your lips.  You are muted!\r\n", ch);
    return;
  }
...
ACMD(do_reply)
{
  ...
  if (GET_LAST_TELL(ch) == NOBODY)
     ...
     * work if someone logss out and back in again.
     */
...
ACMD(do_qcomm)
{
  ...
  if (PLR_FLAGGED(ch, PLR_NOSHOUT)) {
    send_to_char("You cannot quest-say!\r\n", ch);
    return;
  }
